...
function getFieldType(a) {
    ...
    if (b == "INPUT")return a.attr("type"); else if (b == "SELECT")return "select"; else if (b == "TEXTAREA")return "text";
    return ""
}
...
function checkFieldDefault(a, b) {
    ...
    if (b != "select")c = e == defaultValues[d]; else if (e.length != defaultValues[d].length)c = false; else for (var f = 0; f < e.length; f++)if (e[f] != defaultValues[d][f]) {
        c = false;
        break
    }
    return c
...
var validate = {}, validators = {
    _regexp_numeric: /^[0-9]+$/, _regexp_pcre_extract: /(.)(.*)\1(.*)?/, validate_positive_number: function (a) {
...
function savePrefsToLocalStorage(a) {
    ...
    $.ajax({
        ...
        cache: false,
        type: "POST",
        data: {token: a.find("input[name=token]").val(), submit_get_json: true},
        success: function (d) {
    ...
    if (b.length && a) {
        b.find("a").click(function (d) {
            ...
            if ($(this).attr("href") == "#no") {
                b.remove();
                $.post("main.php", {token: b.find("input[name=token]").val(), prefs_autoload: "hide"})
            } else {
