...
function confirmLink(a, b) {
    ...
    if (c)if (typeof a.href != "undefined")a.href += "&is_js_confirmed=1"; else if (typeof a.form != "undefined")a.form.action += "?is_js_confirmed=1";
    return c
}
...
function confirmQuery(a, b) {
    if (PMA_messages.strDoYouReally == "")return true; else if (typeof b.value.replace == "undefined")return true; else {
...
function insertQuery(a) {
    ...
    if (b.options.length > 0) {
        ...
        if (a == "selectall")c = "SELECT * FROM `" + d + "` WHERE 1"; else if (a == "select")c = "SELECT " + e + " FROM `" + d + "` WHERE 1"; else if (a == "insert")c = "INSERT INTO `" + d + "`(" + e + ") VALUES (" + f + ")"; else if (a == "update")c = "UPDATE `" +
        d + "` SET " + j + " WHERE 1"; else if (a == "delete")c = "DELETE FROM `" + d + "` WHERE 1";
    ...
    $("#create_table_form_minimal.ajax").live("submit", function (a) {
        ...
        $.get($form.attr("action"), $form.serialize(), function (e) {
            e.success != undefined && e.success == false ? $('<div id="create_table_dialog"></div>').append(e.error).dialog({
                ...
                height: 230, width: 900, open: PMA_verifyTypeOfAllColumns, buttons: c
