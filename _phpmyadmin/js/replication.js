var random_server_id = Math.floor(Math.random() * 1E7), conf_prefix = "server-id=" + random_server_id + "\nlog-bin=mysql-bin\nlog-error=mysql-bin.err\n";
function update_config() {
    var a = $("#db_select option:selected:first").val();
    $("#db_select option:selected:not(:first)").each(function () {
        a += "," + $(this).val()
    });
    if ($("#db_select option:selected").size() == 0)$("#rep").text(conf_prefix); else $("#db_type option:selected").val() == "all" ? $("#rep").text(conf_prefix + "binlog_ignore_db=" + a) : $("#rep").text(conf_prefix + "binlog_do_db=" + a)
}
