function checkPassword(a) {
    if (typeof a.elements.nopass != "undefined" && a.elements.nopass[0].checked)return true; else if (typeof a.elements.pred_password != "undefined" && (a.elements.pred_password.value == "none" || a.elements.pred_password.value == "keep"))return true;
    var c = a.elements.pma_pw;
    a = a.elements.pma_pw2;
    var b = false;
    if (c.value == "")b = PMA_messages.strPasswordEmpty; else if (c.value != a.value)b = PMA_messages.strPasswordNotSame;
    if (b) {
        alert(b);
        c.value = "";
        a.value = "";
...
$(document).ready(function () {
    ...
    $("#fieldset_add_user a.ajax").live("click", function (a) {
        ...
        $.get($(this).attr("href"), {ajax_request: true}, function (d) {
            $('<div id="add_user_dialog"></div>').prepend(d).find("#fieldset_add_user_footer").hide().end().find("form[name=usersForm]").append('<input type="hidden" name="ajax_request" value="true" />').end().dialog({
                ...
                width: 800, height: 600, modal: true, buttons: b
            });
    ...
    $(".edit_user_anchor.ajax").live("click", function (a) {
        ...
        $.get($(this).attr("href"), {ajax_request: true, edit_user_dialog: true}, function (d) {
            $('<div id="edit_user_dialog"></div>').append(d).dialog({width: 900, height: 600, buttons: b});
            displayPasswordGenerateButton();
            PMA_ajaxRemoveMessage(c)
        })
    });
    ...
    $(".export_user_anchor.ajax").live("click",
        function (a) {
            ...
            $.get($(this).attr("href"), {ajax_request: true}, function (d) {
                $('<div id="export_dialog"></div>').prepend(d).dialog({width: 500, buttons: b});
                PMA_ajaxRemoveMessage(c)
            })
        });
    $("#initials_table.ajax").find("a").live("click", function (a) {
        a.preventDefault();
        var c = PMA_ajaxShowMessage();
        $.get($(this).attr("href"), {ajax_request: true}, function (b) {
