...
$(document).ready(function () {
    ...
    $("#sqlqueryform.ajax input:submit").live("click", function (b) {
        ...
        $.post(a.attr("action"), a.serialize(), function (g) {
            if (g.success == true) {
                ...
                if ($zero_row_results.length > 0)$("#sqlquery").val($zero_row_results.val()); else {
                    $("#sqlqueryresults").show();
            ...
            b.each(function () {
                ...
                if (c.is(":not(.not_null)")) {
                    ...
                    $(".checkbox_null_" + i + "_" + l).bind("click", function () {
                        if (c.is(".enum"))c.find("select").attr("value", ""); else if (c.is(".set"))c.find("select").find("option").each(function () {
                            $(this).attr("selected",
                                false)
    ...
    $(".inline_edit_active span a").live("click", function (b) {
        ...
        g + ")"), k = a.parents("tbody").find("tr").find(".where_clause:nth(" + g + ")").val(); else {
            ...
            if ($this_sibling.is(":not(.relation, .enum, .set)")) {
                ...
                if ($this_sibling.is(".transformed")) {
                    ...
                    typeof g.transformations != "undefined" && $.each(g.transformations, function (q, f) {
                        if (q == r) {
                            if ($this_sibling.is(".text_plain, .application_octetstream"))k = f; else {
                ...
                } else if ($this_sibling.is(".enum"))k = l; else if ($this_sibling.is(".set"))if (l != null) {
                    $.each(l, function (q, f) {
                        k = k + f + ","
                    });
