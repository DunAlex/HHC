function nullify(a, c, b, d) {
    ...
    if (a == 1)c.elements["fields" + d + "[" + b + "]"][1].selectedIndex = -1; else if (a == 2) {
        a = c.elements["fields" + d + "[" + b + "]"];
        if (a.checked)a.checked = false; else {
            b = a.length;
            for (d = 0; d < b; d++)a[d].checked = false
        }
    } else if (a == 3)c.elements["fields" + d + "[" + b + "][]"].selectedIndex = -1; else if (a == 4)c.elements["fields" + d + "[" + b + "]"].selectedIndex = -1; else c.elements["fields" +
    d + "[" + b + "]"].value = "";
    return true
}
...
function isDate(a, c) {
    a = a.replace(/[.|*|^|+|//|@]/g, "-");
    for (var b = a.split("-"), d = 0; d < b.length; d++)if (b[d].length == 1)b[d] = fractionReplace(b[d]);
    a = b.join("-");
    b = 2;
...
function isTime(a) {
    a = a.split(":");
    for (var c = 0; c < a.length; c++)if (a[c].length == 1)a[c] = fractionReplace(a[c]);
    a = a.join(":");
...
function verificationsAfterFieldChange(a, c, b) {
    ...
    $("input[name='fields_null[multi_edit][" + c + "][" + a + "]']").attr({checked: false});
    $("input[name='insert_ignore_" + c + "']").attr({checked: false});
...
$(document).ready(function () {
    ...
    $("#insertFormDEACTIVATED").live("submit", function (a) {
        ...
        $.post(c.attr("action"), c.serialize(), function (b) {
            ...
            } else PMA_ajaxShowMessage(PMA_messages.strErrorProcessingRequest + " : " + b.error, "7000"); else {
                $("#insertForm").remove();
    ...
    $("#insert_rows").live("change", function (a) {
        ...
        if (a < c) {
            for (; a < c;) {
                ...
                PMA_messages.strIgnore + "</label>"); else {
