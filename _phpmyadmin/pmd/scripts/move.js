        ...
        for (key in contr[K])
            for (key2 in contr[K][key])
                for (key3 in contr[K][key][key2]) {
                    if (!document.getElementById("check_vis_" + key2).checked || !document.getElementById("check_vis_" + contr[K][key][key2][key3][0]).checked) continue; // if hide
...
function getColorByTarget(target) {
    ...
    if (color.length == 0) {
        ...
        j = j % 4;
        j++;
        var color_case = [new Array(1, 0, 0),
            new Array(0, 1, 0),
            new Array(0, 0, 1),
            new Array(1, 1, 0),
            new Array(1, 0, 1),
